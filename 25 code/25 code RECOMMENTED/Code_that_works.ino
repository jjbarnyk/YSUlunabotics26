...
#include <MobaTools.h> // additional servo and stepper control lib
#include <Servo.h> // Servo lib

// ----- Wi-Fi Setup -----
// Standard wifi connection stuff, no need to comment
...
#define STEP_SPEED 5000            // Stepper Speed
#define RAMP_LEN   20              // length of ramp ( from stop to actual target speed ) in steps (ease in)
MoToStepper stepper(200, STEPDIR); // declare a stepper with 200 steps/revolution
int currentDirection = 0;          // 1 = forward, -1 = backward, 0 = stop

// ----- Limit switches -----
#define TOP_LIMIT_PIN     8    // Pin of the top limit switch
#define BOTTOM_LIMIT_PIN  7    // Pin of the bottom limit switch
#define LIMIT_ACTIVE      LOW  // NC type: LOW = triggered

// ----- Bucket-servo -----
Servo rotServo1, rotServo2;               //declare the two bucket servos
const int bucketPinL = 6, bucketPinR = 5; // declare the pins for the servos

// ----- Wheel-motor -----
Servo motorA, motorC, motorB, motorD;                // declare the wheel motors as servos?
const int pinA = 9, pinC = 10, pinB = 11, pinD = 12; //pins corresponding to each wheel motor
...
void setup() {
  ...
  //associate the stepper with its previously defined values
  ...
  // --Limit switches--
  // set both limit switch pins to inputs with an active Pull up resistor
